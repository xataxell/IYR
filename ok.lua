--[[
reason i post this is cuz iyr (REAL ONE) got discountinued so its no longer updated
also  be careful as im not sure if its gonna safe or even gonna be updated
(USE AT UR OWN RISK)
feel free to use
also its NOT mine i do NOT own this
i just wanna share and save source on here
]]--

if IY_LOADED and not _G.IY_DEBUG == true then
	loadstring(game:HttpGet('https://raw.githubusercontent.com/f***usfm/infiniteyield-reborn/master/uselessscriptthatexistsinthisrepoiwin'))()
	return
end

-- yeah no there no way to enable this by using a command or somehting. scriptblox is just gay. they have to enable it manually with a one-liner. yes im talking about the is18 s*** 
pcall(function() getgenv().IY_LOADED = true end)

-- f*** you ScriptBlox touches children and love watching little sisters
...
CMDs[#CMDs + 1] = {NAME = 'bang / rape [player] [speed]', DESC = 'F***s someone (NSFW)'}
CMDs[#CMDs + 1] = {NAME = 'facebang / facef*** / facerape [player] [speed]', DESC = 'F***s someone in the face (NSFW)'}
CMDs[#CMDs + 1] = {NAME = 'unbang / unrape / unfacef*** / unfacebang / unfacerape', DESC = 'Stops the rape.'}
...
Players.LocalPlayer.OnTeleport:Connect(function(State)
	if KeepInfYield and (not TeleportCheck) and queueteleport then
		TeleportCheck = true
		queueteleport("loadstring(game:HttpGet('https://raw.githubusercontent.com/f***usfm/infiniteyield-reborn/master/source'))()")
	end
end)
...
addcmd("decompiler", {}, function(args, speaker)
	loadstring(game:HttpGet("https://raw.githubusercontent.com/Edward-Bill/Scripts/main/DecompilerForS***Sploits"))()
end)
...
function updateESP(player, esp)
	...
	if char then
		local cframe = char:GetModelCFrame() --rarely used w**
...
addcmd('headsit',{},function(args, speaker)
	local players = getPlayer(args[1], speaker)
	for i,v in pairs(players) do
		if table.find(devblacklist, tostring(Players[v].UserId)) ~= nil and table.find(devblacklist, tostring(speaker.UserId)) == nil then
...
-- well yeah they dont have anything against perverted s***
...
-- and I. FB_REAL GET F***ING BANNED BECAUSE OF 2 NSFW SCRIPTS ITS NOT EVEN "MORE" ITS JUST 1 EXTRA COMMAND
...
addcmd("facebang", {"facerape","facef***"}, function(args, speaker)
	if args[1] then
		local players = getPlayer(args[1], speaker)
		for _, v in pairs(players) do
			if table.find(devblacklist, tostring(Players[v].UserId)) ~= nil then
...
addcmd("unbang", {"unrape","unfacebang","unfacerape","unfacef***"}, function(args, speaker)
	if bangDied then
		bangDied:Disconnect()
		bang:Stop()
		bangAnim:Destroy()
...
addcmd('emotewheelreplacement',{},function(args, speaker)
	loadstring(game:HttpGet("https://raw.githubusercontent.com/f***usfm/useless-scritps/main/Emotes/main"))()
end)

addcmd('azulawheelreplacement',{},function(args, speaker)
	loadstring(game:HttpGet("https://raw.githubusercontent.com/mireyacs/emoteui/main/source.lua"))()
end)
...
task.spawn(function()
	while task.wait() do
		local success, latestVersionInfo = pcall(function() 
			local versionJson = game:HttpGet('https://raw.githubusercontent.com/f***usfm/infiniteyield-reborn/master/version')
